...
(function() {
  ...
  if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    ...
    window.ananoscoin.ananosjs.AnanodeApi = exports;
  }
})();
// FINISHED BOTTOM nodejs/browser hack
'use strict';
...
(function() {
  ...
  if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    ...
    window.ananoscoin.ananosjs.loggingUtil = exports;
  }
})();
// FINISHED BOTTOM nodejs/browser hack
'use strict';
...
(function() {
  ...
  if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    ...
    window.ananoscoin.ananosjs.sleepUtil = exports;
  }
})();
// FINISHED BOTTOM nodejs/browser hack
'use strict';
...
(function() {
  ...
  const getRawStrFromMajorAmountStr = (amountStr, amountPrefix) => {
    ...
    // console.log('STARTED getRawStrFromAmountStr', ananosStr, decimalPlace, divisor);
    if (decimalPlace !== -1) {
    ...
    // console.log('INTERIM getRawStrFromAmountStr', ananosStr, decimalPlace, divisor);
    const amountBi = BigInt(amountStr);
    // console.log('INTERIM getRawStrFromAmountStr ananos   ', ananos);
    // console.log('INTERIM getRawStrFromAmountStr ananosDiv', majorDivisor);
    ...
    // console.log('INTERIM getRawStrFromAmountStr ananosRaw', ananosRaw);
    // const parts = getAmountPartsFromRaw(ananosRaw.toString());
    // console.log('SUCCESS getRawStrFromAmountStr', ananosStr, ananosRaw, parts);
   ...
   *
   * @memberof AnanosUtil
   * @param {string} amountStr the anaoshi, as a string.
  ...
  /**
   * @typedef {Object} ananosParts
   * @property {string} ananos - The amount of ananos.
   * @property {string} anaoshi - The amount of anaoshi (not counting whole ananos).
   * @property {string} raw - The amount of raw (not counting whole ananos and whole anaoshi).
   */
  ...
  const getAmountPartsFromRaw = (amountRawStr, amountPrefix) => {
    ...
    //    console.log(`ananosRaw:    ${ananosRaw}`);
    const prefixDivisor = prefixDivisors[amountPrefix];
    const majorDivisor = prefixDivisor.majorDivisor;
    const minorDivisor = prefixDivisor.minorDivisor;
    //    console.log(`ananosDivisor:   ${ananosDivisor}`);
    const major = amountRaw / majorDivisor;
    //    console.log(`ananos:${ananos}`);
    ...
    const ananosParts = {};
    ananosParts.majorName = prefixDivisor.majorName;
    ananosParts.minorName = prefixDivisor.minorName;
    ananosParts[prefixDivisor.majorName] = major.toString();
    ananosParts[prefixDivisor.minorName] = minor.toString();
    ananosParts.raw = amountRawRemainder.toString();
    return ananosParts;
  };
  ...
  const getAccountPublicKey = (account) => {
    if (account === undefined) {
      throw Error(`Undefined ANANOS Account`);
    }
    ...
    if (account.startsWith('camo')) {
      if (
        ...
        throw Error(`Invalid CAMO ANANOS Account prefix '${account}'`);
      }
      ...
      if (
        (!account.startsWith('ana_1') && !account.startsWith('ana_3')) ||
        account.length !== 64
      ) {
        throw Error(`Invalid ANANOS Account prefix '${account}'`);
      }
    ...
    if (!isAccountValid.valid) {
      throw Error(
          `Invalid ANANOS Account '${account}', ${isAccountValid.message}`,
      );
    }
   ...
   * @memberof AnanosUtil
   * @param {string} publicKey the public key.
   * @param {string} accountPrefix the prefix. ana_ or nano_.
  ...
  const hashMessageToBytes = (message) => {
    ...
    const context = blake.blake2bInit(32, null);
    // ananosMessagePreamble is technically not needed for dummy blocks but helps separate Nano signing from Ananos signing
    blake.blake2bUpdate(context, utf8ToBytes(ananosMessagePreamble));
  ...
  const getPrivateKey = (seed, seedIx) => {
    ...
    if (!isValid.valid) {
      throw Error(`Invalid ANANOS seed '${seed}', ${isValid.message}`);
    }
  ...
  const sendFromPrivateKeyWithRepresentativeAndPrevious = async (
    ...
    const accountInfo = await AnanodeApi.getAccountInfo(accountAddress);
    ...
    if (newRepresentative !== undefined) {
      ...
      representative = await AnanodeApi.getAccountRepresentative(
          accountAddress,
      );
    }
    ...
    if (previous == '') {
      ...
      const work = await AnanodeApi.getGeneratedWork(previous);
      block.work = work;
      /* istanbul ignore if */
      if (LOG_SEND) {
      ...
      const processResponse = await AnanodeApi.process(block, 'send');
      /* istanbul ignore if */
      if (LOG_SEND || LOG_SEND_PROCESS) {
  ...
  const open = async (
    ...
    const work = await AnanodeApi.getGeneratedWork(publicKey);
    const accountAddress = getAccount(publicKey, accountPrefix);
    const block = {};
    block.type = 'state';
    block.account = accountAddress;
    ...
    try {
      const processResponse = await AnanodeApi.process(block, 'open');
      /* istanbul ignore if */
      if (LOG_OPEN) {
  ...
  const change = async (
    ...
    try {
      const processResponse = await AnanodeApi.process(block, 'change');
      /* istanbul ignore if */
      if (LOG_CHANGE) {
  ...
  const receive = async (
    ...
    try {
      const processResponse = await AnanodeApi.process(block, 'receive');
      /* istanbul ignore if */
      if (LOG_RECEIVE) {
  ...
  */
  const getAnanosAccountValidationInfo = (account) => {
    if (account === null) {
      return {
    ...
    if (account.length == 64) {
      if (!account.startsWith('ana_1') && !account.startsWith('ana_3')) {
        return {
          message:
            'Invalid ANANOS Account (does not start with ana_1 or ana_3)',
          valid: false,
        };
      }
    } else {
    ...
    if (!isValid) {
      return {
        message: `Invalid ANANOS account (characters after ana_ must be one of:13456789abcdefghijkmnopqrstuwxyz)`,
        valid: false,
      };
    }
    ...
    try {
      ...
      return {
        message: `Invalid ANANOS account (${error.message})`,
        valid: false,
      };
    }
    return {
  ...
  const isAccountOpen = async (AnanodeApi, account) => {
  ...
  const exports = (() => {
    ...
    exports.getAnanosAccountValidationInfo = getAnanosAccountValidationInfo;
  ...
  if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    ...
    window.ananoscoin.ananosjs.AnanosUtil = exports;
  }
})();
// FINISHED BOTTOM nodejs/browser hack
'use strict';
...
(function() {
  ...
  const sweep = async (
    ...
    const account = AnanosUtil.getAccount(publicKey, accountPrefix);
    ...
    if (historyHistory.length == 0) {
      ...
      for (
        ...
        if (
          ...
          const accountBalanceRaw = await AnanodeApi.getAccountBalanceRaw(
              account,
              accountPrefix,
          );
  ...
  if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    ...
    window.ananoscoin.ananosjs.depositUtil = exports;
  }
})();
// FINISHED BOTTOM nodejs/browser hack
'use strict';
