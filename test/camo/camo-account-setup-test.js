...
describe('camo-account-setup', () => {
  ...
  it('open-account', async () => {
    const fundingPrivateKey = AnanosUtil.getPrivateKey(
    ...
    const account0 = AnanosUtil.getAccount(publicKey0, 'ana_');
    console.log('privateKey0', privateKey0, account0);

    const publicKeyF = AnanosUtil.getPublicKey(privateKeyF);
    const accountF = AnanosUtil.getAccount(publicKeyF, 'ana_');
  ...
  it('test-shared-seed', async () => {
    ...
    const publicKey0 = AnanosUtil.getPublicKey(privateKey0);
    const account0 = AnanosUtil.getAccount(publicKey0, 'ana_');
    const representative0 = await AnanodeApi.getAccountRepresentative(
      account0
    );
    const camoPublicKey0 = AnanosUtil.getAccountPublicKey(representative0);
    ...
    const publicKeyF = AnanosUtil.getPublicKey(privateKeyF);
    const accountF = AnanosUtil.getAccount(publicKeyF, 'ana_');
    const representativeF = await AnanodeApi.getAccountRepresentative(
      accountF
    );
    const camoPublicKeyF = AnanosUtil.getAccountPublicKey(representativeF);
  ...
  it('test-send-get-balance', async () => {
    ...
    const amountRaw = AnanosUtil.getRawStrFromMajorAmountStr('6');
    ...
    for (let seedIx = 0; seedIx < hashes.length; seedIx++) {
      ...
      const privateKey = AnanosUtil.getPrivateKey(
