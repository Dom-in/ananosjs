...
const getPrivateKey0Bytes = () => {
  const privateKey0 = AnanosUtil.getPrivateKey(camoTestData.seed0, 0);
  const privateKey0Bytes = AnanosUtil.hexToBytes(privateKey0);
...
describe('camo-keys', () => {
  it('public-key', async () => {
    ...
    const privateKeyF = AnanosUtil.getPrivateKey(camoTestData.seedF, 0);

    const publicKey0 = AnanosUtil.getPublicKey(privateKey0);
  ...
  it('camo-public-key', async () => {
    ...
    const privateKeyF = AnanosUtil.getPrivateKey(camoTestData.seedF, 0);
    const publicKeyF = AnanosUtil.getPublicKey(privateKeyF);

    const privateKey0Bytes = AnanosUtil.hexToBytes(privateKey0);
    const privateKeyFBytes = AnanosUtil.hexToBytes(privateKeyF);
  ...
  it('ananos-nacl scalarMult.base vs scalarbase', async () => {
    /**
     * Ananos/Nano hashes the private key before using it to derive a public key (nacl.js derivePublicFromSecret).
     * <p>
  ...
  it('camo-shared-seed', async () => {
    ...
    const actualSharedSeed = AnanosUtil.getPrivateKey(secret0F, 0);
