...
describe('camo', () => {
  it('getCamoPublicKey account matches expected', async () => {
    const ananosjs = testUtil.getAnanosjsWithCamoApi();
    const expectedResponse =
      '80989F0ED4154E886AD926392DB1F9B524AB250A1BEEBA6C999A2F06C36F7E00';
    const actualResponse = await ananosjs.getCamoPublicKey(privateKey0);
    expect(expectedResponse).to.deep.equal(actualResponse);
  });
  it('getSharedSecret account matches expected', async () => {
    const ananosjs = testUtil.getAnanosjsWithCamoApi();
    const publicKey0 = await ananosjs.getCamoPublicKey(privateKey0);
    const publicKey1 = await ananosjs.getCamoPublicKey(privateKey1);
    const sharedSecret01 = await ananosjs.getSharedSecret(
        privateKey0,
        publicKey1,
    );
    const sharedSecret10 = await ananosjs.getSharedSecret(
        privateKey1,
        publicKey0,
    );
  ...
  it('camo account matches expected', async () => {
    const ananosjs = testUtil.getAnanosjsWithCamoApi();
    const publicKey0 = await ananosjs.getCamoPublicKey(privateKey0);
    const actualResponse = await ananosjs.getCamoAccount(publicKey0);
    const expectedResponse =
  ...
  coinDatas.forEach((coinData) => {
    it(coinData.coin + ' camoGetAccountsPending camo error', async () => {
      ...
      const camoGetAccountsPending =
        coinData.getCamoGetAccountsPendingFn(ananosjs);
    ...
    it(coinData.coin + ' camoGetAccountsPending camo error', async () => {
      ...
      const message = `Invalid CAMO ANANOS Account \'${invalidCamoAccount}\', does not match regex '^[13456789abcdefghijkmnopqrstuwxyz]+$'`;
      const camoGetAccountsPending =
        coinData.getCamoGetAccountsPendingFn(ananosjs);
      await testUtil.expectErrorMessage(
          message,
    ...
    it(coinData.coin + ' getCamoSharedAccountData camo error', async () => {
      ...
      const getCamoSharedAccountData =
        coinData.getCamoSharedAccountDataFn(ananosjs);
      await testUtil.expectErrorMessage(
          message,
    ...
    it(
        ...
        async () => {
          ...
          const message = `Invalid CAMO ANANOS Account length 66 of \'${invalidCamoAccount}\'`;
          const getCamoSharedAccountData =
          coinData.getCamoSharedAccountDataFn(ananosjs);
          await testUtil.expectErrorMessage(
              message,
    ...
    it(coinData.coin + ' receive account matches expected', async () => {
      ...
      const camoReceive = coinData.getCamoReceiveFn(ananosjs);
    ...
    it(coinData.coin + ' receive account matches expected', async () => {
      ...
      expectedResponse.push(
          '98A4831A04AE621D5AAAA004858DDB44EF98C4F520CF702BEAC942C5EB54530F',
      );
      expectedResponse.push(
          '08EFF77F9D9D2EA1F3DAAC1844B353E9E760ACB1B7F48D07F0C0DD8BC8108C7C',
      );
      expectedResponse.push(
    ...
    it(
        ...
        async () => {
          ...
          if (coinData.coin == 'ananos') {
            ...
            expectedResponse.push(
                'C7771A83A9BABFF0E8555D2EA241DE79959C8C520519AB806ECA610B2AD2B748',
            );
            expectedResponse.push(
                '6B1D491C0A93CF509CC13F9964F4B2C6273FCD96469E3189873AB529EF0D7FAB',
            );
          }
          if (coinData.coin == 'nano') {
            expectedResponse.push(
                '2C91AAFA9FF589ED3723E5A01E11046ED335701585F232F779B437F706D2502D',
            );
            expectedResponse.push(
                '3A5BB8A7912F58953FF7FB5EA5FD089ED5F4715321B83CADCFA39C1FDA9E01B5',
            );
          }
          const camoSend = coinData.getCamoSendFn(ananosjs);
    ...
    it(coinData.coin + ' receive account matches expected', async () => {
      ...
      const getCamoAccountBalanceRaw =
        coinData.getCamoAccountBalanceRawFn(ananosjs);
    ...
    it(
        ...
        async () => {
          ...
          coinData.getCamoGetNextPrivateKeyForReceiveFn(ananosjs);
    ...
    it(
        ...
        async () => {
          ...
          if (coinData.coin == 'ananos') {
            ...
            expectedResponse.push(
                '825CE38C924FC857EE9D49282CCB7F18132549BB5F0DE56914812FB216FAFD1A',
            );
          }
          if (coinData.coin == 'nano') {
            expectedResponse.push(
                'BAF3A8F66639550359A8E79438302854D5A8584849EAC320467F06C526FD816D',
            );
            expectedResponse.push(
    ...
    it(
        ...
        async () => {
          ...
          if (coinData.coin == 'ananos') {
            ...
            expectedResponse.push(
                'EE77AC5D02D350289E62885D861664046D38EA729EFA3EDABB6D0FD6012609DE',
            );
          }
          if (coinData.coin == 'nano') {
            expectedResponse.push(
    ...
    it(
        ...
        async () => {
          ...
          const amountAnanos = '1';
          const publicKey0 = await ananosjs.getCamoPublicKey(privateKey0);
          const camoAccount0 = await ananosjs.getCamoAccount(publicKey0);
          const expectedResponse = [];
          if (coinData.coin == 'ananos') {
            expectedResponse.push(
          ...
          if (coinData.coin == 'nano') {
            expectedResponse.push(
                '07166C6071108B28EB3099EB69C26BF469D976D9E45FD512E0FD54E01C09DD08',
            );
          }
          const camoSendWithdrawalFromSeed =
          coinData.getCamoSendWithdrawalFromSeedFn(ananosjs);
    ...
    it(
        ...
        async () => {
          const ananosjs =
          testUtil.getAnanosjsWithAccountRepresentativeUndefinedApi();
          const count = 1;
          const publicKey0 = await ananosjs.getCamoPublicKey(privateKey0);
          const camoAccount0 = await ananosjs.getCamoAccount(publicKey0);
    ...
    it(
        ...
        async () => {
          ...
          const expectedResponse = {
            ...
            sharedPrivateKey:
            'EEB5604BC2C6F5E39100F380F21B8AD61FB9F4BAC96E56AB74DFABE8348891F8',
            sharedPublicKey:
            '47F614F94E9DFAEE3CA71D69CBAA2B6642F2C1B17C1AE668EF8D902364173427',
            sharedSeed:
    ...
    it(coinData.coin + ' camo receiveCamoBlock valid response', async () => {
      ...
      const receiveCamoDepositsForSeed =
        coinData.getReceiveCamoDepositsForSeedFn(ananosjs);
