...
const signAndVerify = (privateKey, publicKey, hash) => {
  const signature = ananosjs.signHash(privateKey, hash);
  const verified = ananosjs.verify(hash, signature, publicKey);
  return verified;
};
...
describe('split-key-camo', () => {
  it('test split key', () => {
    ...
    const publicKey0 = ananosjs.getCamoPublicKey(privateKey0);
    // console.log('publicKey0', publicKey0.length, publicKey0);

    const publicKey1 = ananosjs.getCamoPublicKey(privateKey1);
