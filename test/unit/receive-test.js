  ...
  coinDatas.forEach((coinData) => {
    ...
    it(
        ...
        async () => {
          ...
          coinData.getReceiveDepositsForSeedFn(ananosjs);
    ...
    it(coinData.coin + ' receiveDepositsForSeed errors', async () => {
      ...
      const messages = {
        ananos:
          'getAccountsPending accounts:ana_3i1aq1cchnmbn9x5rsbap8b15akfh7wj7pwskuzi7ahz8oq6cobd99d4r3b7 count:10',
    ...
    it(coinData.coin + ' receiveDepositsForSeed process fork', async () => {
      const ananosjs = testUtil.getAnanosjsWithProcessForkApi();
    ...
    it(coinData.coin + ' receiveDepositsForSeed pending error', async () => {
      const ananosjs = testUtil.getAnanosjsWithPendingErrorApi();
      const receiveDepositsForSeed =
        coinData.getReceiveDepositsForSeedFn(ananosjs);
      const actualResponse = await receiveDepositsForSeed(
          seed0,
