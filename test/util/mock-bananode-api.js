...
const getBalanceRaw = (account) => {
  for (let ix = 0; ix < ananosjs.PREFIXES.length; ix++) {
    const prefix = ananosjs.PREFIXES[ix];
    if (account.startsWith(prefix)) {
      return AnanosUtil.getRawStrFromMajorAmountStr('10', prefix);
    }
  }
};
...
const getAccountsBalances = (accounts) => {
  // https://docs.nano.org/commands/rpc-protocol/#accounts-balances
  const json = {};
  json.balances = {};
  json.balances[accounts[0]] = {};
  json.balances[accounts[0]].balance = getBalanceRaw(accounts[0]);
  json.balances[accounts[0]].pending = '000123000456000789000000000000';
  json.balances[accounts[1]] = {};
  json.balances[accounts[1]].balance = getBalanceRaw(accounts[1]);
  json.balances[accounts[1]].pending = '000123000456000789000000000000';
  const balances = json;
...
const getPrevious = (account) => {
  ...
  if (account.startsWith('ana_1bad1')) {
    return '';
  }
  const retval = {};
  retval.frontiers = {};
  retval.frontiers[account] =
...
const process = async (block, subtype) => {
  ...
  const retval = {};
  const blockHash = AnanosUtil.hash(block);
...
const getGeneratedWork = async (hash) => {
  ...
  if (
    hash == 'C008B814A7D269A1FA3C6528B19201A24D797912DB9996FF02A1FF356E45552B'
  ) {
    defaultWork = 'F5E2210000000000';
  }
  if (
    hash == 'E30D22B7935BCC25412FC07427391AB4C98A4AD68BAA733300D23D82C9D20AD3'
  ) {
    defaultWork = '5DAD3C0000000000';
  }
  if (
    hash == '6C22D9056710A1EC9D61D9A0BCB45CA77B1757D651D0113A845A0E53C10F4E4F'
  ) {
    defaultWork = '23922D0000000000';
  }
  if (
    hash == '08ADFBD2D47434DCEA55C903395B2C4219C16D860A630518EBAEBD79C8E295E0'
  ) {
    defaultWork = '10A51C0100000000';
  }
  if (
    hash == '2C91AAFA9FF589ED3723E5A01E11046ED335701585F232F779B437F706D2502D'
  ) {
    defaultWork = '980C430000000000';
  }
  if (
    hash == 'BAF3A8F66639550359A8E79438302854D5A8584849EAC320467F06C526FD816D'
  ) {
    defaultWork = 'A807110000000000';
  }
  if (
    hash == '60C3B3D9DD0AF49D590E8AFC448690FC9CAF8E6911641C3A40AB16DB33F8F038'
  ) {
    defaultWork = 'FA1E1C0000000000';
  }
  if (
    hash == 'C7771A83A9BABFF0E8555D2EA241DE79959C8C520519AB806ECA610B2AD2B748'
  ) {
  ...
  if (GENERATE_UNKNOWN_BLOCK_WORK) {
    if (defaultWork == undefined) {
      const work = AnanosUtil.getHashCPUWorker(
          hash,
          AnanosUtil.getZeroedWorkBytes(),
      );
      throw Error(
    ...
    const workBytes = AnanosUtil.hexToBytes(defaultWork).reverse();
    const hashBytes = AnanosUtil.hexToBytes(hash);
    const isWorkValid = AnanosUtil.isWorkValid(hashBytes, workBytes);
...
const getAccountsPending = async (accounts, count) => {
  ...
  validPendingAccountSet.add(
      'ana_3jfbronhgapg9usdisp5rt4ioh65aajzp8woryt4jpxpakgpi5syfx96khed',
  );
  validPendingAccountSet.add(
      'ana_3rrf6cus8pye6o1kzi5n6wwjof8bjb7ff4xcgesi3njxid6x64pms6onw1f9',
  );
  validPendingAccountSet.add(
      'ana_39y66s786kbejeyohok53jfx3qoc78bapqc3hec8qgrswjrjskefqyhjrjsc',
  );
  validPendingAccountSet.add(
      'ana_1w8shy6om7ts74piy619x3aqpxb96nmc476p7mh59absweoicnrg5wqmz1kd',
  );
  validPendingAccountSet.add(
      'ana_1dzcca9ycmtx3q79mocmu95zdduxptp3gp5fqkmb1ownscpweggzah8cb4rb',
  );
  validPendingAccountSet.add(
      'ana_1jzp4mwnx9htxrycg9dbsgo4psk4yd1u4z1twsngz5ei6fk3gf395w8ponjs',
  );
  validPendingAccountSet.add(
...
const getAccountHistory = async (account, count, head, raw) => {
  ...
  block.height = '60';
  block.hash =
    '80392607E85E73CC3E94B4126F24488EBDFEB174944B890C97E8F36D89591DC5';
  return retval;
};
