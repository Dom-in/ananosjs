...
const bytesToHex = ananosjs.AnanosUtil.bytesToHex;

const hexToBytes = ananosjs.AnanosUtil.hexToBytes;
...
describe('vanity', () => {
  describe('ananos', async () => {
    ...
    const ananosToCamoPublicKeyMap = new Map();
    const canmToAnanosPublicKeyMap = new Map();
    ...
    const getPublicKey = async (secret) => {
      const ananosPublicKey = await ananosjs.AnanosUtil.getPublicKey(secret);
      const camoPublicKey = await ananosjs.camoUtil.getCamoPublicKey(secret);

      const ananosPublicKeyCheck = edToCurve(ananosPublicKey);
      expect(camoPublicKey).to.deep.equal(ananosPublicKeyCheck);

      const key = await eced25519.keyFromPrivate(secret);
      const publicKey = key.getPublic();
      const camoPublicKeyCheck = toXYPoint(publicKey);

      console.log('getPublicKey', ananosPublicKey, camoPublicKeyCheck);
      ananosToCamoPublicKeyMap.set(ananosPublicKey, camoPublicKeyCheck);
      canmToAnanosPublicKeyMap.set(
        camoPublicKeyCheck.toString(),
        ananosPublicKey
      );
      return ananosPublicKey;
    };
    ...
    const publicKeyAdd = async (a, b) => {
      ...
      const aCurve = ananosToCamoPublicKeyMap.get(a);
      const bCurve = ananosToCamoPublicKeyMap.get(b);
      ...
      const c = canmToAnanosPublicKeyMap.get(cPoint.toString());
    ...
    it('random', async () => {
      ...
      const signature = ananosjs.AnanosUtil.signHash(ab, msg);
      const verified = await ananosjs.AnanosUtil.verify(msg, signature, AB);
