...
const getNewSeed = async (ix) => {
  ...
  const seed = window.ananoscoinAnanosjs.AnanosUtil.bytesToHex(seedBytes);
...
const getAccountHistoryAndPending = async (ix) => {
  ...
  const publicKey = await window.ananoscoinAnanosjs.getPublicKey(privateKey);
...
const getAccountHistory = async (ix, account) => {
  window.ananoscoinAnanosjs.setAnanodeApiUrl(url);
  const history = await window.ananoscoinAnanosjs.getAccountHistory(
    account,
    maxHistory
  );
...
const getAccountsPending = async (ix, account) => {
  window.ananoscoinAnanosjs.setAnanodeApiUrl(url);
  const pending = await window.ananoscoinAnanosjs.getAccountsPending(
    [account],
    maxPending,
    true
  );
...
const getCamoRepresentative = (ix, seed) => {
  const privateKey = window.ananoscoinAnanosjs.getPrivateKey(seed, 0);
  const camoPublicKey = window.ananoscoinAnanosjs.getCamoPublicKey(privateKey);
  const camoRepresentative =
    window.ananoscoinAnanosjs.getCamoAccount(camoPublicKey);
  document.getElementById('camoRepresentative' + ix).innerText =
    camoRepresentative;
};

const getAnanosAccountDeposits = async (ix) => {
  const seed = document.getElementById('seed' + ix).value;
  const privateKey = window.ananoscoinAnanosjs.getPrivateKey(seed, 0);
  const publicKey = window.ananoscoinAnanosjs.getPublicKey(privateKey);
  const account = window.ananoscoinAnanosjs.getAnanosAccount(publicKey);
  const response = await window.ananoscoinAnanosjs.receiveAnanosDepositsForSeed(
    seed,
    0,
    account
  );
...
const changeRepresentativeToCamo = async (ix, seed) => {
  ...
  const representative =
    window.ananoscoinAnanosjs.getAnanosAccount(camoPublicKey);
  if (accountInfo.representative) {
    if (accountInfo.representative != representative) {
      ...
      const response =
        await window.ananoscoinAnanosjs.changeAnanosRepresentativeForSeed(
          seed,
          0,
          representative
        );
...
const getAnaAsCamo = (banAccount) => {
  if (banAccount) {
    // console.log('STARTED getAnaAsCamo', banAccount);
    const publicKey = window.ananoscoinAnanosjs.getAccountPublicKey(banAccount);
    const camoAccount = window.ananoscoinAnanosjs.getCamoAccount(publicKey);
    // console.log('SUCCESS getAnaAsCamo', banAccount, camoAccount);
    return camoAccount;
  }
};
...
const setSharedSeed = async () => {
  ...
  const accountInfoRepresentativeAsCamo1 = getAnaAsCamo(
    accountInfo1.representative
  );
  const accountInfoRepresentativeAsCamo2 = getAnaAsCamo(
  ...
  const account1AsCamo = getAnaAsCamo(account1);
  const account2AsCamo = getAnaAsCamo(account2);

  const sharedData12 =
    await window.ananoscoinAnanosjs.getCamoAnanosSharedAccountData(
      seed1,
      0,
      account2AsCamo,
      0
